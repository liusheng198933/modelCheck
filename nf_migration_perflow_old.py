...
rule_actionx1 = [Implies(And(X[j][m] == i+1, m < srcNFV[j]), And(Action0[i][j][0] == X[j][m+1], HighRule0[i][j][0] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]
rule_actionx2 = [Implies(And(X[j][m] == i+1, m == srcNFV[j]), And(Action0[i][j][0] == nodeNum+2*j+1, HighRule0[i][j][0] > 0, Action0[i][j][1] == X[j][m+1], HighRule0[i][j][1] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]
rule_actionx3 = [Implies(And(X[j][m] == i+1, m > srcNFV[j]), And(Action0[i][j][1] == X[j][m+1], HighRule0[i][j][1] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]

rule_actiony1 = [Implies(And(Y[j][m] == i+1, m < destNFV[j]), And(Action1[i][j][0] == Y[j][m+1], HighRule1[i][j][0] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]
...
rule_actiony3 = [Implies(And(Y[j][m] == i+1, m > destNFV[j]), And(Action1[i][j][1] == Y[j][m+1], HighRule1[i][j][1] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]
...
rule_dst = [ Implies(nodeNum == j + 1, And(Action0[j][i][1] == nodeNum, Action1[j][i][1] == nodeNum)) for i in range(flowNum) for j in range(nodeNum)]
...
path_0 = simplify(And([Implies(And(P[h][i] == X[h][j], X[h][j] == m+1, PROS[h][i] == fg, PTMP[h][i] <= RuleTMP0[m][h][fg], PTMP[h][i] <= RuleTMP1[m][h][fg]), Or(PACT[h][i+1] == 0, PACT[h][i+1] == 1)) for fg in range(2) for m in range(nodeNum) for h in range(flowNum) for i in range(pathMaxLength-1) for j in range(pathLength)]))

path_1 = simplify(And([Implies(And(P[h][i] == Y[h][j], Y[h][j] == m+1, PROS[h][i] == fg, PTMP[h][i] <= RuleTMP0[m][h][fg], PTMP[h][i] <= RuleTMP1[m][h][fg]), Or(PACT[h][i+1] == 0, PACT[h][i+1] == 1)) for fg in range(2) for m in range(nodeNum) for h in range(flowNum) for i in range(pathMaxLength-1) for j in range(pathLength)]))
...
path_2 = simplify(And([Implies(And(P[h][i] == X[h][j], X[h][j] == m+1, PROS[h][i] == fg, PTMP[h][i] <= RuleTMP1[m][h][fg], PTMP[h][i] > RuleTMP0[m][h][fg]), PACT[h][i+1] == 1) for fg in range(2) for m in range(nodeNum) for h in range(flowNum) for i in range(pathMaxLength-1) for j in range(pathLength)]))

path_3 = simplify(And([Implies(And(P[h][i] == Y[h][j], Y[h][j] == m+1, PROS[h][i] == fg, PTMP[h][i] <= RuleTMP1[m][h][fg], PTMP[h][i] > RuleTMP0[m][h][fg]), PACT[h][i+1] == 1) for fg in range(2) for m in range(nodeNum) for h in range(flowNum) for i in range(pathMaxLength-1) for j in range(pathLength)]))

path_4 = [Implies(And(P[h][i] == nodeNum+2*h+1, PROS[h][i] == 0, srcNFV[h] == m, X[h][m] == n+1), And(P[h][i+1] == X[h][m], Or(And(PTMP[h][i+1] == RuleTMP0[n][h][1], PROS[h][i+1] == 1), And(PTMP[h][i+1] == newTag, PROS[h][i+1] == 0)))) for m in range(pathLength) for n in range(nodeNum-1) for h in range(flowNum) for i in range(pathMaxLength-1) ]

path_5 = [Implies(And(P[h][i] == nodeNum+2*h+2, PROS[h][i] == 0, destNFV[h] == m), And(P[h][i+1] == Y[h][m], PROS[h][i+1] == 1, PTMP[h][i+1] == newTag)) for m in range(pathLength) for h in range(flowNum) for i in range(pathMaxLength-1) ]

forward1 = simplify(And([Implies(And(P[h][i] == m+1, PACT[h][i] == 0, PROS[h][i] == fg), And(P[h][i+1] == Action0[m][h][fg], PTMP[h][i+1] == TagTMP0[m][h][fg], PROS[h][i+1] == fg)) for fg in range(2) for h in range(flowNum) for m in range(nodeNum) for i in range(pathMaxLength-1)]))

forward2 = simplify(And([Implies(And(P[h][i] == m+1, PACT[h][i] == 1, PROS[h][i] == fg), And(P[h][i+1] == Action1[m][h][fg], PTMP[h][i+1] == TagTMP1[m][h][fg], PROS[h][i+1] == fg)) for fg in range(2) for h in range(flowNum) for m in range(nodeNum) for i in range(pathMaxLength-1)]))
...
#rule_actiony2 = [Implies(And(Y[j][m] == i+1, m == dstNFV[j]), And(Action1[i][j][0] == nodeNum+2*j+2, HighRule1[i][j][0] > 0, Action1[i][j][1] == Y[j][m+1], HighRule1[i][j][1] > 0)) for i in range(nodeNum) for j in range(flowNum) for m in range(pathLength-1)]
...
# Backward closure
#bug_fix1 = simplify(And([Implies( And(Y[i][m1] == h1 + 1, Action0[h1][i] == Y[i][m2], Y[i][m2] == h2 + 1, RuleTMP1[h2][i] == newTag, And([Y[i][m1] != X[i][n] for n in range(pathLength)])), TagTMP1[h1][i] == newTag) for i in range(flowNum) for m1 in range(pathLength) for m2 in range(pathLength) for h1 in range(nodeNum) for h2 in range(nodeNum)]))

#bug_fix2 = simplify(And([Implies( And(Y[i][m1] == h1 + 1, Action0[h1][i] == Y[i][m2], Y[i][m2] == h2 + 1, RuleTMP1[h2][i] == newTag, Y[i][m1] == X[i][n2], Or(m1 != n2, Not(And([Y[i][m3] == X[i][m3] for m3 in range(m1) ]))) ), TagTMP1[h1][i] == newTag) for i in range(flowNum) for m1 in range(pathLength) for m2 in range(pathLength) for h1 in range(nodeNum) for h2 in range(nodeNum) for n1 in range(pathLength) for n2 in range(pathLength)]))
...
s.add(path_0, path_1, path_2, forward1, forward2)
s.add(path_4 + path_5)
