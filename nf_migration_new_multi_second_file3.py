...
result_path = '/result/result_3.txt'
...
bug_fix2 = [Implies(And(NFSeqY[i][j] == k, PROS[i][h][j] == 0, P[i][h] == Z[i][m], P[i][h] == Y[i][n], k < n), \
                         PACT[i][h] == 0) for i in range(flowNum) for j in range(seqNum) for k in range(pathLength) for h in range(pathMaxLength) for m in range(midPathLength) for n in range(pathLength)]
