...
bug_fix2 = [Implies(And(NFSeqX[i][j] == k1, NFSeqY[i][j] == k2, PROS[i][h][j] == 0, P[i][h] == Z[i][m], P[i][h] == X[i][n], k1 > n, And([Z[i][c] != Y[i][k2] for c in range(m, midPathLength)])), \
                         PACT[i][h] == 0) for i in range(flowNum) for j in range(seqNum) for k1 in range(pathLength) for k2 in range(pathLength) for h in range(pathMaxLength) for m in range(midPathLength) for n in range(pathLength)]
