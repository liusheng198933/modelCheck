...
PVER = [[ Int("pver_%s_%s" % (i+1, j+1)) for i in range(pathMaxLength) ] for j in range(flowNum)]
...
pact_range1 = [ And(0 <= POVER[i][j], POVER[i][j] <= verNum) for i in range(flowNum) for j in range(pathMaxLength)]

pact_range2 = [ And(0 <= PVER[i][j], PVER[i][j] <= verNum) for i in range(flowNum) for j in range(pathMaxLength)]
...
path_range1 = [ And(0 <= PO[i][j], PO[i][j] <= nodeNum + 2 * flowNum) for i in range(flowNum) for j in range(pathMaxLength)]

path_range2 = [ And(0 <= P[i][j], P[i][j] <= nodeNum + 2 * flowNum) for i in range(flowNum) for j in range(pathMaxLength)]

path_neq1 = [ Implies(PO[i][j] == PO[i][j+1], Or(PO[i][j] == dstNode[i], PO[i][j] == 0)) for i in range(flowNum) for j in range(pathMaxLength-1)]

path_neq2 = [ Implies(P[i][j] == P[i][j+1], Or(P[i][j] == dstNode[i], P[i][j] == 0)) for i in range(flowNum) for j in range(pathMaxLength-1)]
...
drop2 = [Implies(P[h][i] == 0, And([P[h][i+j+1] == 0 for j in range(pathMaxLength-1-i)])) for h in range(flowNum) for i in range(pathMaxLength-1)]
...
rule_dst2 = [Implies(P[j][h] == dstNode[j], P[j][h+1] == dstNode[j]) for h in range(pathMaxLength-1) for j in range(flowNum)]
...
forward2 = [Implies(And(P[j][h] == k+1, P[j][h+1] == n+1, PVER[j][h+1] == v), \
                    P[j][h+2] == RACT[j][n][k][v]) for v in range(verNum+1) for j in range(flowNum) for k in range(nodeNum+2*flowNum) for n in range(nodeNum + 2*flowNum) for h in range(pathMaxLength-2)]
...
rule_actionx2 = [Implies(And(NFSeqX[i][j] == k, X[i][k] == P[i][h], PVER[i][h] == 0), \
...
# bug_fix3 = [Implies(And(PO[j][k] == X[j][m], NFSeqY[j][i] == p, POVER[j][k] == 1, PROS[j][a][i] == 0, \
#                         PO[j][k] == P[j][a], PO[j][k-1] == P[j][a-1], And([PO[j][c] != Y[j][p] for c in range(k, pathMaxLength)])),
#                    PVER[j][a] == 0)
#                    for j in range(flowNum) for a in range(1, pathMaxLength-1) for k in range(1, pathMaxLength-1) for i in range(seqNum) for m in range(pathLength) for p in range(pathLength)]

forward3 = [Implies(And(POVER[j][k] == 1, PO[j][k] == P[j][a], PO[j][k-1] == P[j][a-1], \
                    And([Implies(And(PROS[j][a][i] == 0, NFSeqY[j][i] == p), Or([PO[j][c] == Y[j][p] \
                        for c in range(k, pathMaxLength)])) for i in range(seqNum) for p in range(pathLength)])), \
                    PVER[j][a] == POVER[j][k])
                    for j in range(flowNum) for k in range(1, pathMaxLength-1) for a in range(1, pathMaxLength-1) ]


forward4 = [Implies(And(PO[j][k] == P[j][a], PO[j][k-1] == P[j][a-1], POVER[j][k] == 0), \
                     PVER[j][a] == 0) for j in range(flowNum) for a in range(1, pathMaxLength-1) for k in range(1, pathMaxLength-1) ]




bug_fix3 = [Implies(And(POVER[j][k] == 1, PO[j][k] == P[j][a], PO[j][k-1] == P[j][a-1], \
                        Or([And(PROS[j][a][i] == 0, NFSeqY[j][i] == p, And([PO[j][c] != Y[j][p] for c in range(k, pathMaxLength)])) \
                            for i in range(seqNum) for p in range(pathLength)])), \
                    PVER[j][a] == 0) for j in range(flowNum) for a in range(1, pathMaxLength-1) for k in range(1, pathMaxLength-1) ]



# forward3 = [Implies(And(P[j][k] == PO[j][a], P[j][k-1] == PO[j][a-1]), \
#                     PVER[j][k] == POVER[j][a]) \
#                     for j in range(flowNum) for k in range(1, pathMaxLength-1) for a in range(1, pathMaxLength-1)]
...
path_init = [P[i][0] == srcNode[i] for i in range(flowNum)]
path_sec = [P[i][1] == X[i][1] for i in range(flowNum)]
...
# goal = [Or([And(PO[j][k] == X[j][m], NFSeqY[j][i] == p, POVER[j][k] == 1, \
#             And([PO[j][c] != Y[j][p] for c in range(k, pathMaxLength)])) \
#             for k in range(1, pathMaxLength-1) for m in range(pathLength) for p in range(pathLength)]) for j in range(flowNum)]

#goal = [ Or([PO[j][k] != P[j][k] for k in range(pathMaxLength)]) for j in range(flowNum) ]
...
s.add(pact_range1 + pact_range2 + pros_range + nfv_order0 + nfv_order1 + nfv_range0 + nfv_range1 + nfv_dst0 + nfv_dst1)
...
s.add(pver_ascd + path_range1 + path_range2 + drop1 + drop2 + rule_actionx1 + rule_actionx2 + rule_dst1 + rule_dst2)
s.add(forward1 + forward2 + forward3 + forward4 + rver_init + rule_actionz2 + rule_actionz3 + rule_actionz4 + rule_actionz5 + pros_change1 + pros_change2)
s.add(init_x + init_y + init_z + final_x + final_y + final_z)
s.add(path_org_init + path_org_sec + path_init + path_sec + pros_init + path_neq1 + path_neq2)
...
if s.check() == sat:
    ...
    for i in range(flowNum):
        ...
        r = [ m.evaluate(PVER[i][h]) for h in range(pathMaxLength) ]
        print 'Our packet action:'
        print_matrix(r)
