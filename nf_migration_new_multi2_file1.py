...
result_path = '/result/result2_1.txt'
...
forward3 = [Implies(And(Not(And(PO[j][a] == X[j][m], NFSeqY[j][i] == p, POVER[j][a] == 1, PROS[j][k][i] == 0, \
                            And([PO[j][c] != Y[j][p] for c in range(a, pathMaxLength)]))), \
                        P[j][k] == PO[j][a], P[j][k-1] == PO[j][a-1]), \
                    PVER[j][k] == POVER[j][a]) \
                    for j in range(flowNum) for k in range(1, pathMaxLength-1) for a in range(1, pathMaxLength-1) for i in range(seqNum) for m in range(pathLength) for p in range(pathLength)]
...
s.add(forward1 + forward2 + forward3 + rver_init + rule_actionz2 + rule_actionz3 + rule_actionz4 + rule_actionz5 + pros_change1 + pros_change2)
