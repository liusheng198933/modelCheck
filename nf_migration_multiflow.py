...
rule_actionx4 = [Implies(And(srcNFV[j] == pathLength-1, destNode[j] == m+1), And(Action0[m][j][0] == nodeNum+2*j+1, Action0[m][j][1] == m+1)) for m in range(nodeNum) for j in range(flowNum) ]
...
rule_actiony4 = [Implies(And(destNFV[j] == pathLength-1, destNode[j] == m+1), And(Action1[m][j][0] == nodeNum+2*j+2, HighRule1[m][j][0] <= ruleNum, Action1[m][j][1] == m+1, HighRule1[m][j][1] <= ruleNum)) for m in range(nodeNum) for j in range(flowNum) ]
...
rule_dst = [ And(destNode[i] == m+1, Action0[m][i][1] == m+1, Action1[m][i][1] == m+1) for m in range(nodeNum) for i in range(flowNum) ]
...
bug_fix2 = simplify(And([Implies( And(Y[i][m1] == h1 + 1, Action0[h1][i][fg] == Y[i][m2], Y[i][m2] == h2 + 1, RuleTMP1[h2][i][fg] == newTag, Y[i][m1] == X[i][n], Or(m1 != n, Not(And([Y[i][m3] == X[i][m3] for m3 in range(m1) ]))) ), TagTMP1[h1][i][fg] == newTag) for fg in range(2) for i in range(flowNum) for m1 in range(pathLength) for m2 in range(pathLength) for h1 in range(nodeNum) for h2 in range(nodeNum) for n in range(pathLength)]))
