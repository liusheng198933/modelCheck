...
result_path = '/result/result_4.txt'
...
path_neq = [ Implies(P[i][j] == P[i][j+1], Or(P[i][j] == dstNode[i], P[i][j] == 0)) for i in range(flowNum) for j in range(pathMaxLength-1)]
...
rule_actionx1 = [Implies(And(X[j][m] == P[j][h], PACT[j][h] == 0), P[j][h+1] == X[j][m+1]) for h in range(pathMaxLength-1) for j in range(flowNum) for m in range(pathLength-1)]


rule_actionx2 = [Implies(And(NFSeqX[i][j] == k, X[i][k] == P[i][h], PACT[i][h] == 0), \
...
bug_fix3 = [Implies(And(P[j][k] == X[j][m], NFSeqY[j][i] == p, PACT[j][k] == 1, PROS[j][k][i] == 0, k < n, \
                    And([And([P[j][h] != Z[j][q] for q in range(midPathLength)]) for h in range(k+1, n)]), \
                    Or([P[j][n] == Z[j][b] for b in range(midPathLength)])),
                    #Or(k+1 == n, And([And([P[j][h] != Z[j][q] for q in range(midPathLength)]) for h in range(k+1, n)]))), \
                    Or([P[j][c] == Y[j][p] for c in range(n, pathMaxLength)]))
...
no_blackhole = And([P[i][pathMaxLength-1] == dstNode[i] for i in range(flowNum)])
...
s.add(pact_range + pact_const + pros_range + nfv_order0 + nfv_order1 + nfv_range0 + nfv_range1 + nfv_dst0 + nfv_dst1)
...
s.add(rule_actionz1 + rule_actionz2 + rule_actionz3 + rule_actionz4 + rule_actionz5 + pros_change1 + pros_change2)
s.add(init_x + init_y + init_z + final_x + final_y + final_z)
s.add(path_init + pros_init + path_neq + path_new)
