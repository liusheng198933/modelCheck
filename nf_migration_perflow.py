...
nfSeq = [[ Int("nfseq_%s_%s" % (i+1, j+1)) for i in range(seqNum) ] for j in range(flowNum)]
...
nfSeq_range = [ And(nfSeq[i][j]) for i in range(flowNum) for j in range(seqNum)]
nfSeq_not_equal = [ Implies() for i in range(flowNum) for j in range(seqNum)]
